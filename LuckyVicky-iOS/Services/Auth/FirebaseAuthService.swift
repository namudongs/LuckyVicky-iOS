...
final class FirebaseAuthService: AuthService {
  ...
  func signIn(with credential: ASAuthorizationAppleIDCredential) async throws -> User {
    ...
    if let authorizationCode = credential.authorizationCode {
      do {
        ...
        // If refresh token handling fails (e.g., already revoked), continue sign-in flow
  ...
  private func storeRefreshToken(_ codeString: String) async throws {
    let urlString =
      "https://us-central1-luckyvicky-ios.cloudfunctions.net/getRefreshToken?code=\(codeString)"
  ...
  private func revokeRefreshToken(_ token: String) async throws {
    let urlString =
      "https://us-central1-luckyvicky-ios.cloudfunctions.net/revokeToken?refresh_token=\(token)"
      .addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
