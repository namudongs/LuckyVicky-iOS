...
extension ChatGPTService {
  static func createDefault() -> ChatGPTService {
    ...
    let configuration = AIServiceConfiguration(
      ...
      systemPrompt:
        """
        Turn negative thoughts into super positive thinking using emojis by:
        1. Maintaining logical connection to the original problem
        2. Suggesting realistic and practical alternatives
        3. Avoiding unrealistic scenarios
        4. Relating specifically to the mentioned situation

        Keep informal and friendly tone, end with "완전 럭키비키잔앙🍀".
        Provide exactly one response in 2-3 sentences maximum.
